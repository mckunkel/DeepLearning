...
public class SparkMLDecisionTreesDemo {
	...
	public static void main(String[] args) {
		...
		Cleanse Data
		--------------------------------------------------------------------------*/
		// Convert all data types as double; Change missing values to standard
		// ones.

		// Create the schema for the data to be loaded into Dataset.
		StructType irisSchema = DataTypes.createStructType(
		...
		JavaRDD<Row> rdd2 = rdd1.map(new Function<Row, Row>() {
			...
			public Row call(Row iRow) throws Exception {
				...
				Row retRow = RowFactory.create(Double.valueOf(iRow.getString(0)), Double.valueOf(iRow.getString(1)),
						Double.valueOf(iRow.getString(2)), Double.valueOf(iRow.getString(3)), iRow.getString(4));
		...
		StringIndexer indexer = new StringIndexer().setInputCol("SPECIES").setOutputCol("IND_SPECIES");
		...
		JavaRDD<LabeledPoint> rdd4 = rdd3.map(new Function<Row, LabeledPoint>() {
			...
			public LabeledPoint call(Row iRow) throws Exception {
				...
				LabeledPoint lp = new LabeledPoint(iRow.getDouble(5),
						Vectors.dense(iRow.getDouble(0), iRow.getDouble(1), iRow.getDouble(2), iRow.getDouble(3)));
