...
public class SparkSQLDemo {
	...
	public static void main(String[] args) {
		...
		Dataset<Row> summaryData = empDf.groupBy(col("deptid")).agg(avg(empDf.col("salary")), max(empDf.col("age")));
		summaryData.show();
		...
		Dataset<Row> deptDf = spSession.createDataFrame(deptList, Department.class);
		System.out.println("Contents of Department DF : ");
		deptDf.show();
		deptDf.groupBy(col("id")).count().show();
		...
		empDf.filter(col("age").gt(30)).join(deptDf, col("deptid").equalTo(col("id"))).groupBy(col("deptid"))
				.agg(avg(empDf.col("salary")), max(empDf.col("age"))).show();
		empDf.filter(col("age").gt(30)).groupBy(col("deptid")).agg(avg(empDf.col("salary")), max(empDf.col("age")))
				.show();
		// Create Data Frames from an CSV
		Dataset<Row> autoDf = spSession.read().option("header", "true").csv("data/auto-data.csv");
		autoDf.show(5);
		...
		demoDf.select("region", "sector", "superlayer", "status_change_type", "locwire").show();
		demoDf.select("runno").sort(asc("runno")).distinct().show();

		// demoDf.show();

		// iris homework: load iris.csv in a dataframe called irisDF
		// in irisDF, filter for rows whose petalWidth is greater than 0.5 and
		// count them

		Dataset<Row> irisDf = spSession.read().option("header", "true").csv("data/iris.csv");
		irisDf.show();
		irisDf.filter(col("PetalWidth").gt(0.4)).groupBy("Species").count().show();
		irisDf.createOrReplaceTempView("IRIS");

		spSession.sql("SELECT SPECIES, max(SepalWidth), avg(PetalLength) " + "FROM IRIS GROUP BY SPECIES").show();
		spSession.sql("SELECT SPECIES, max(SepalWidth), avg(PetalWidth) " + "FROM IRIS GROUP BY SPECIES").show();
